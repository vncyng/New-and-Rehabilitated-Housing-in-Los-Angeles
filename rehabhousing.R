...
# STATS 141SL
# rehabhousing.R
# Purpose: To clean data for rehab housing
...
# Extract Altered/Repaired Building Permits, for 1 or 2 family dwellings, or Apartments
data <- raw_data[as.character(raw_data$Permit.Type)=='Bldg-Alter/Repair',]
...
summary(data$longitude)                           #still 5750 missing values, merge with Assessor Parcels Data
...
summary(data$longitude) # -118.7 to -118.2        #still 1179 missing values, use census geocoding


#============================ Using Census geocoding to retrieve Latitude/Longitudes: ========================================================
# Find rows with missing lat and lon
missing_latlong_ID <- which(is.na(data$latitude)) # row indices of missing lat/longs, there are 1179
# Use https://geocoding.geo.census.gov/
write.csv(cbind(data$Address[missing_latlong_ID],data$Zip.Code[missing_latlong_ID]),"geocode_rehab.csv")
geocode_results <- read.csv("GeocodeResults_rehab.csv",blank.lines.skip=FALSE,header=FALSE)
...
summary(data$latitude) # 33.71 to 34.33
summary(data$longitude) # -118.7 to -118.2          #still 492 missing values, use google maps api
...
# Find rows with missing lat and lon
missing_latlong_ID <- which(is.na(data$latitude)) # row indices of missing lat/longs, there are 492
...
summary(data$latitude) # 33.71 to 34.33
summary(data$longitude) # -118.7 to -118.2          # missing 99 values only now

rehab <- data
write.csv(data,"rehabhousing.csv") 


# Still has missing values, rerun geocode sometime without Los Angeles and see if it helps...
